...
public class SulfurSpikeBlock
    ...
    public SulfurSpikeBlock(AbstractBlock.Settings settings) {
        super(settings);
    ...
    private static void place(WorldAccess world, BlockPos pos, Direction direction, Thickness thickness) {
        BlockState blockState = (BlockState)((BlockState)((BlockState) ModBackportBlocks.SULFUR_SPIKE.getDefaultState().with(VERTICAL_DIRECTION, direction)).with(THICKNESS, thickness)).with(WATERLOGGED, world.getFluidState(pos).getFluid() == Fluids.WATER);
    ...
    private static BlockPos getTipPos(BlockState state2, WorldAccess world, BlockPos pos2, int range, boolean allowMerged) {
        ...
        BiPredicate<BlockPos, BlockState> biPredicate = (pos, state) -> state.isOf(ModBackportBlocks.SULFUR_SPIKE) && state.get(VERTICAL_DIRECTION) == direction;
    ...
    public static boolean canDrip(BlockState state) {
        return isPointingDown(state) && state.get(THICKNESS) == Thickness.TIP && state.get(WATERLOGGED) == false;
    }
    ...
    private static Optional<BlockPos> getSupportingPos(World world, BlockPos pos2, BlockState state2, int range) {
        Direction direction = state2.get(VERTICAL_DIRECTION);
        BiPredicate<BlockPos, BlockState> biPredicate = (pos, state) -> state.isOf(ModBackportBlocks.SULFUR_SPIKE) && state.get(VERTICAL_DIRECTION) == direction;
        return searchInDirection(world, pos2, direction.getOpposite().getDirection(), biPredicate, state -> !state.isOf(ModBackportBlocks.SULFUR_SPIKE), range);
    }
    ...
    private static boolean isTip(BlockState state, boolean allowMerged) {
        if (!state.isOf(ModBackportBlocks.SULFUR_SPIKE)) {
            return false;
        }
        Thickness thickness = state.get(THICKNESS);
    ...
    private static boolean isHeldByPointedDripstone(BlockState state, WorldView world, BlockPos pos) {
        return isPointingDown(state) && !world.getBlockState(pos.up()).isOf(ModBackportBlocks.SULFUR_SPIKE);
    }
    ...
    private static boolean isPointedDripstoneFacingDirection(BlockState state, Direction direction) {
        return state.isOf(ModBackportBlocks.SULFUR_SPIKE) && state.get(VERTICAL_DIRECTION) == direction;
    }

    @Nullable
    private static BlockPos getCauldronPos(World world, BlockPos pos2, Fluid fluid) {
        Predicate<BlockState> predicate = state -> false;
    ...
    public static BlockPos getDripPos(World world, BlockPos pos2) {
        BiPredicate<BlockPos, BlockState> biPredicate = (pos, state) -> canDripThrough(world, pos, state);
        return searchInDirection(world, pos2, Direction.UP.getDirection(), biPredicate, SulfurSpikeBlock::canDrip, 11).orElse(null);
    }

    public static Fluid getDripFluid(ServerWorld world, BlockPos pos) {
        return getFluid(world, pos, world.getBlockState(pos)).map(fluid -> fluid.fluid).filter(SulfurSpikeBlock::isFluidLiquid).orElse(Fluids.EMPTY);
    }
    ...
    private static boolean canGrow(BlockState dripstoneBlockState, BlockState waterState) {
        return dripstoneBlockState.isOf(ModBackportBlocks.SULFUR) && waterState.isOf(Blocks.WATER) && waterState.getFluidState().isStill();
    }
