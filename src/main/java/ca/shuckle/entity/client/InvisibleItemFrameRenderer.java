...
public class InvisibleItemFrameRenderer<T extends ItemFrameEntity>
    ...
    protected int getBlockLight(T itemFrameEntity, BlockPos blockPos) {
        if (((Entity)itemFrameEntity).getType() == ModEntities.INVIS_GLOW_ITEM_FRAME) {
    ...
    public void render(T itemFrameEntity, float f, float g, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i) {
        ...
        //this is the part that makes the actual frame render, idfk how to make it use custom textures
        /**
        if (!bl) {
    ...
    private int getLight(T itemFrame, int glowLight, int regularLight) {
        return ((Entity)itemFrame).getType() == ModEntities.INVIS_GLOW_ITEM_FRAME ? glowLight : regularLight;
    }

    private ModelIdentifier getModelId(T entity, ItemStack stack) {
        boolean bl;
        boolean bl2 = bl = ((Entity)entity).getType() == ModEntities.INVIS_GLOW_ITEM_FRAME;
    ...
    public Identifier getTexture(T itemFrameEntity) {
        return new Identifier(ShuckleQOL.MOD_ID, "textures/block/invisible_block.png");
    }
