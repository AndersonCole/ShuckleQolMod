...
import ca.shuckle.screen.custom.slot.CrafterOutputSlot;
...
public class CrafterScreenHandler extends ScreenHandler implements ScreenHandlerListener {
    ...
    public CrafterScreenHandler(int syncId, PlayerInventory playerInventory, PacketByteBuf buf) {
        ...
        this.inventory = new CraftingInventory(this, CrafterBlockEntity.GRID_WIDTH, CrafterBlockEntity.GRID_HEIGHT);
        this.addSlots(playerInventory);
        this.pos = buf.readBlockPos();
    }

    public CrafterScreenHandler(int syncId, PlayerInventory playerInventory, CraftingInventory inventory, PropertyDelegate propertyDelegate) {
        super(ModScreenHandlers.CRAFTER_3X3, syncId);
        this.player = playerInventory.player;
        this.propertyDelegate = propertyDelegate;
        this.inventory = inventory;
        checkSize(inventory, CrafterBlockEntity.GRID_SIZE);
    ...
    private void addSlots(PlayerInventory playerInventory) {
        ...
        for(i = 0; i < 3; ++i) {
            for(j = 0; j < 3; ++j) {
                int k = j + i * 3;
                this.addSlot(new CrafterInputSlot(this.inventory, k, 26 + j * 18, 17 + i * 18, this));
            }
        }
    ...
    private void updateResult() {
        if (this.player instanceof ServerPlayerEntity serverPlayerEntity) {
            ...
            ItemStack itemStack = CrafterBlock.getCraftingRecipe(world, this.inventory).map((recipe) ->
                    recipe.craft(this.inventory, world.getRegistryManager())).orElse(ItemStack.EMPTY);
